...
LLM_MODEL_NAME = "nvidia/OpenCodeReasoning-Nemotron-1.1-7B"
LLM_GGUF_FILE = ""
RAG_CACHE_DIRNAME = ".rag_cache_code_plus_plus"
...
LLM_GPU_MEM_UTIL = float(os.getenv("RAG_GPU_MEM_UTIL", "0.88"))
LLM_MAX_MODEL_LEN = int(os.getenv("RAG_MAX_MODEL_LEN", "4096"))
LLM_CPU_OFFLOAD_GB = float(os.getenv("RAG_CPU_OFFLOAD_GB", "0"))

DOC_PREFIX = "search_document: "
QUERY_PREFIX = "search_query: "
SESSION_LOG_FILENAME = "RAGLLM_Code_Plus_Plus_Session.txt"
...
class RAGEngine:
    ...
    def _load_llm(self) -> None:
        _is_gguf = bool(LLM_GGUF_FILE)
        model_dir = _ensure_local_model(
            LLM_MODEL_NAME, "llm", self.status_cb,
            allow_patterns=[LLM_GGUF_FILE] if _is_gguf else None,
        )
        ...
        try:
            if _is_gguf:
                if Llama is None:
                    raise RuntimeError("llama-cpp-python is required for GGUF models. Install: pip install llama-cpp-python")
                ...
                if LLM is None:
                    raise RuntimeError("vllm is required for non-GGUF models. See PROJECT.md")
                ...
                try:
                    ...
                    self.status_cb("LLM loaded on CUDA:0.")
                except Exception as gpu_exc:
                    if not _is_oom_error(gpu_exc):
                        raise
                    _stop.set()
                    self.status_cb("__llm_load_pct__:0")
                    self.status_cb("Full-GPU load exceeded VRAM budget. Retrying with reduced utilization + CPU offload.")
                    _stop = self._start_llm_load_monitor(model_dir)
                    engine = LLM(
                        model=str(model_dir),
                        dtype=dtype,
                        gpu_memory_utilization=max(0.50, LLM_GPU_MEM_UTIL - 0.15),
                        max_model_len=LLM_MAX_MODEL_LEN,
                        cpu_offload_gb=16.0,
                        trust_remote_code=True,
                        enforce_eager=True,
                ...
                self.llm_tokenizer = engine.get_tokenizer()
    ...
    def _generate(self, prompt: str, max_new_tokens: int, temperature: float) -> str:
        ...
        if Llama is not None and isinstance(self.llm_engine, Llama):
...
class RAGApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.top_k_var = tk.StringVar(master=self.root, value="5")
        self.max_tokens_var = tk.StringVar(master=self.root, value="4096")
        self.temp_var = tk.StringVar(master=self.root, value="0.1")
